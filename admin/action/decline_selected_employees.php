...
if (isset($_POST['user_ids']) && is_array($_POST['user_ids'])) {
    ...
    // SQL query to update the 'status' of multiple users to 'DECLINED'
    $sql = "UPDATE user_tbl SET status = 'DECLINED' WHERE user_id IN ($user_ids_string)";

    // Execute the query
    if (mysqli_query($con, $sql)) {
        // Fetch the contact numbers for the DECLINED users
        $contact_query = "SELECT contact_no FROM employee_tbl WHERE user_id IN ($user_ids_string)";
