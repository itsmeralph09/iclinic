...
<body id="page-top">
    ...
    <script>
        $(document).ready(function() {
            ...
            // Delegate click event handling to a parent element
            $(document).on('click', '[id^="addStock_"]', function(e) {
                e.preventDefault(); // Prevent default form submission
                var userID = $(this).attr('id').split('_')[1]; // Extract event ID
                var formData = $('#addStockForm_' + userID); // Get the form data
                var modalDiv = $('#plus_' + userID);
                ...
                if (fieldsAreValid) {
                    $.ajax({
                        url: 'action/add_item_stock.php', // URL to submit the form data
                        type: 'POST',
                        data: formData.serialize(), // Form data to be submitted
                        dataType: 'json',
                        success: function(response) {
                        ...
                        error: function(xhr, status, error) {
                            // Handle the error response
                            console.error(xhr.responseText); // Output error response to console (for debugging)
                            showSweetAlert('error', 'Error', 'Failed to add stock. Please try again later.');
                        }
                    });
                }
            });
